...
class Algorithm(digdag.BaseTask):
    def zlib(self):
        with open("output.Z", "w") as f:
            f.write("zzzzzzzzzz")
        self.export_params["path_zlib"] = "output.Z"
        self.export_params["size_zlib"] = 10

    def deflate(self):
        with open("output.gz", "w") as f:
            f.write("defldefl")
        self.export_params["path_deflate"] = "output.gz"
        self.export_params["size_deflate"] = 8

    def bzip2(self):
        with open("output.bzip2", "w") as f:
            f.write("bz2bz2")
        self.export_params["path_deflate"] = "output.bz2"
        self.export_params["size_deflate"] = 6
    ...
    def decide_algorithm(self, **params):
        ...
        self.export_params["best_path"] = best_path
