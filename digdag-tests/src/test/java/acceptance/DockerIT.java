...
public class DockerIT
{
    ...
    @Test
    public void verifyDockerPullOptions()
            throws Exception
    {
        final Path tempdir = folder.getRoot().toPath().toAbsolutePath();
        final Path projectDir = tempdir.resolve("docker");
        ...
        assertThat(initStatus.code(), is(0));
        Files.createDirectories(scriptsDir);
        copyResource("acceptance/docker/docker_pull_options.dig", projectDir.resolve("docker_pull_options.dig"));
