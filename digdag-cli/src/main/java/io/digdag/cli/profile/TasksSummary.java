 ...
 *
 * Current implementation has a few limitations of task start delay analysis:
 *
 * 1. Skip whole task analysis when an attempt:
 *   1. contains `error` tasks
 *
 *   2. contains `_checked` directive
 *
 * 2. Skip a task analysis when the task doesn't have `started_at`
 *    (this can happen with old archived tasks generated by old implementation)
 *
 * 3. Skip a task analysis when the task has no upstream tasks and:
 ...
 *      (this can happen with old archived tasks generated by old implementation)
 *
 * 4. Skip a task analysis when the task has any upstream tasks
 *    and the upstream tasks don't have `started_at` (this happens when the attempt is started by `retry --resume`)
 *
 * 5. Skip a task analysis when the task is retried one because a retried task is directly connected to the parent task
            ...
            ArchivedTask task)
    {
        ...
        if (!isRoot && task.getStartedAt().isPresent()) {
            ...
            if (task.getUpstreams().isEmpty()) {
                ...
                if (parentId.isPresent()) {
                    ...
                    if (previousTask.getTaskType().isGroupingOnly()) {
                        ...
                        // Give up analysis of task start delay if the parent is not purely group task
                ...
                // and it doesn't work for this analysis.
            ...
            if (shouldProfileStartDelay
                    ...
                    // Task start delay analysis on retried task is skipped
